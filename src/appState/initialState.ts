...
let initialState: AppContextState = {
  dispatch: () => {},
  playing: {
    index: 0,
    progress: 27000,
...
export const FAV_TRACKS_LOCAL_STORAGE_KEY = 'USER-FAV-TRACKS'
export const RECENT_TRACKS_LOCAL_STORAGE_KEY = 'USER-RECENT-TRACKS'
export const ALL_TRACKS_LOCAL_STORAGE_KEY = 'ALL-TRACKS-VAPE'
...
export async function getLocalStorageTracks() {
  ...
  const savedAllTracks = await Storage.getObject<Song[]>({
    key: ALL_TRACKS_LOCAL_STORAGE_KEY,
  })
  //   const savedFavs = localStorage.getItem(FAV_TRACKS_LOCAL_STORAGE_KEY);
  //   const savedRecent = localStorage.getItem(RECENT_TRACKS_LOCAL_STORAGE_KEY);
  if (savedFavs && savedFavs.length > 0) {
    // favTracks = JSON.parse(savedFavs);
    favTracks = savedFavs
  }
  if (savedRecent && savedRecent.length > 0) {
    // recentTracks = JSON.parse(savedRecent);
    recentTracks = savedRecent
  }
  if (savedAllTracks && savedAllTracks.length > 0) {
    allTracks = savedAllTracks
  }
  return {
    favTracks,
    recentTracks,
...
export function setLocalStorageAllTracks(tracks: Song[]) {
  ...
  // localStorage.setItem(RECENT_TRACKS_LOCAL_STORAGE_KEY, JSON.stringify(tracks))
}
...
export function setLocalStorageFaveTracks(tracks: Song[]) {
  ...
  // localStorage.setItem(FAV_TRACKS_LOCAL_STORAGE_KEY, JSON.stringify(tracks))
}

// getInitialState(initialState).then(updated => {
//   initialState = updated
// })

export async function getInitialState(initialState: AppContextState) {
  await getLocalStorageTracks().then(
    ({ favTracks, recentTracks, allTracks }) => {
      ...
      if (allTracks.length > 0) {
        initialState.music.tracks = allTracks
      }
      const music = getMusic(initialState, initialState.music.tracks)
      if (music.hotTracks.length > 0) {
        initialState.music.hotTracks = music.hotTracks
      }
      if (music.newTracks.length > 0) {
        initialState.music.newTracks = music.newTracks
      }
      if (music.tracks.length > 0) {
        initialState.music.tracks = music.tracks
      }
    }
  )
  return initialState
